...
module.exports = {
  config: {
    ...
    role: 0,
    author: "AjiroDesu",
    description: "Put a photo or user's avatar on an ad billboard.",
    category: "fun",
  ...
  onStart: async function ({ api, message, event, usersData, args }) {
    ...
    try {
      ...
      if (attachedImageUrl) {
        ...
        if (!imageUrl) {
          return message.reply(
            "📢 **How to use /ad (FB Page non-admin only):**\n" +
            "1️⃣ Send a photo in the conversation.\n" +
            "2️⃣ Reply to that photo with the command: `ad`\n" +
      ...
      const tempPath = __dirname + "/ad_temp.png";
