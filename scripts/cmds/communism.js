...
module.exports = {
  config: {
    ...
    role: 0,
    author: "AjiroDesu",
    description: "Apply a communism overlay to a photo or a user's avatar.",
    category: "fun",
  ...
  onStart: async function ({ api, message, event }) {
    ...
    try {
      ...
      if (attachedImageUrl) {
        ...
        if (!imageUrl) {
          return message.reply(
            "📌 **How to use /communism (FB Page non-admin only):**\n" +
            "1️⃣ Send a photo in the conversation.\n" +
            "2️⃣ Reply to that photo with the command: `communism`\n" +
      ...
      const tempPath = __dirname + "/communism_temp.png";
