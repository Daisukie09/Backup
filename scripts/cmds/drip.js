...
module.exports = {
  config: {
    ...
    role: 0,
    author: "AjiroDesu",
    description: "Give a photo or a user's avatar the drip treatment.",
    category: "fun",
  ...
  onStart: async function ({ api, message, event }) {
    ...
    try {
      ...
      if (attachedImageUrl) {
        ...
        if (!imageUrl) {
          return message.reply(
            "💧 **How to use /drip (FB Page non-admin only):**\n" +
            "1️⃣ Send a photo in the conversation.\n" +
            "2️⃣ Reply to that photo with the command: `drip`\n" +
      ...
      const apiUrl = `https://api.popcat.xyz/v2/drip?image=${encodeURIComponent(imageUrl)}`;
      const res = await axios.get(apiUrl, { responseType: "arraybuffer", timeout: 30000 });
      const buffer = Buffer.from(res.data);

      const tempPath = __dirname + "/drip_temp.png";
