...
module.exports = {
  config: {
    ...
    role: 0,
    author: "AjiroDesu",
    description: "Put a photo or a user's avatar on a Nokia screen.",
    category: "fun",
  ...
  onStart: async function ({ api, message, event }) {
    ...
    try {
      ...
      if (attachedImageUrl) {
        ...
        if (!imageUrl) {
          return message.reply(
            "📱 **How to use /nokia (FB Page non-admin only):**\n" +
            "1️⃣ Send a photo in the conversation.\n" +
            "2️⃣ Reply to that photo with the command: `nokia`\n" +
      ...
      const tempPath = __dirname + "/nokia_temp.png";
